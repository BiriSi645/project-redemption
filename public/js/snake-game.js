(() => {
    ...
    function reset() {
        ...
        score = 0; level = 1; running = false; paused = false; ended = false;
        food = createFood();
        scoreElement.textContent = '0'; levelElement.textContent = '1';
        pauseButton.disabled = true; pauseButton.textContent = 'Duraklat'; startButton.disabled = false;
        showOverlay('Oyuna hazır mısınız?', 'Başlat düğmesine basın veya yön tuşlarından birini kullanın.');
        draw();
    }

    function createFood() {
        const empty = [];
        for (let y=0;y<gridSize;y++) for (let x=0;x<gridSize;x++) if (!snake?.some(part=>part.x===x&&part.y===y)) empty.push({x,y});
    ...
    function tick() {
        ...
        if (head.x<0||head.x>=gridSize||head.y<0||head.y>=gridSize||bodyToCheck.some(part=>part.x===head.x&&part.y===head.y)) return gameOver();
        snake.unshift(head);
        if (ate) {
            score += 10; level = Math.min(9, Math.floor(score/50)+1); food = createFood();
    ...
    function setDirection(name, autoStart=true) {
        const next = directions[name]; if (!next) return;
    ...
    async function saveScore() {
        ...
        try{
            const response=await fetch(root.dataset.scoreUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','X-Requested-With':'XMLHttpRequest'},body});
            const data=await response.json();if(!response.ok||!data.success)throw new Error(data.message||'Skor kaydedilemedi.');
            best=Number(data.personalBest)||best;bestElement.textContent=String(best);renderLeaderboard(data.leaderboard);
            scoreStatus.textContent=data.improved?'Yeni kişisel rekorunuz kaydedildi!':'Skor kaydedildi; kişisel rekorunuz değişmedi.';
        }catch(error){scoreStatus.textContent='Skor şu anda kaydedilemedi.';}
    }
    function renderLeaderboard(entries){leaderboardElement.innerHTML='';if(!entries.length){const empty=document.createElement('li');empty.className='leaderboard-empty';empty.textContent='Henüz kayıtlı skor yok.';leaderboardElement.appendChild(empty);return;}const medals=['🥇','🥈','🥉'];entries.forEach((entry,index)=>{const item=document.createElement('li'),rank=document.createElement('span'),player=document.createElement('span'),value=document.createElement('strong');rank.className='leaderboard-rank';rank.textContent=medals[index];player.className='leaderboard-player';player.textContent=entry.username;value.textContent=`${entry.score} puan`;item.append(rank,player,value);leaderboardElement.appendChild(item);});}
    function showOverlay(title,text){overlayTitle.textContent=title;overlayText.textContent=text;overlay.hidden=false;}
    function draw() {
        context.fillStyle='#07140d';context.fillRect(0,0,canvas.width,canvas.height);
        context.strokeStyle='rgba(255,255,255,.035)';context.lineWidth=1;
        for(let i=1;i<gridSize;i++){context.beginPath();context.moveTo(i*cell,0);context.lineTo(i*cell,canvas.height);context.stroke();context.beginPath();context.moveTo(0,i*cell);context.lineTo(canvas.width,i*cell);context.stroke();}
        context.fillStyle='#ef4444';context.beginPath();context.arc((food.x+.5)*cell,(food.y+.5)*cell,cell*.34,0,Math.PI*2);context.fill();
        snake.forEach((part,index)=>{context.fillStyle=index===0?'#4ade80':'#22c55e';const inset=index===0?2:3;context.fillRect(part.x*cell+inset,part.y*cell+inset,cell-inset*2,cell-inset*2);});
    }
    ...
    document.addEventListener('keydown',event=>{const map={ArrowUp:'up',w:'up',W:'up',ArrowDown:'down',s:'down',S:'down',ArrowLeft:'left',a:'left',A:'left',ArrowRight:'right',d:'right',D:'right'};if(map[event.key]){event.preventDefault();setDirection(map[event.key]);}else if(event.code==='Space'){event.preventDefault();togglePause();}});
    canvas.addEventListener('touchstart',event=>{const t=event.changedTouches[0];touchStart={x:t.clientX,y:t.clientY};},{passive:true});
