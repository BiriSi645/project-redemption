...
{
    private const CHANCE_CARDS=[['text'=>'Başlangıca ilerle ve ₺200 al.','action'=>'move','value'=>0],['text'=>'Banka temettüsü: ₺50 al.','action'=>'money','value'=>50],['text'=>'Üç kare geri git.','action'=>'back','value'=>3],['text'=>'Hapse git.','action'=>'jail'],['text'=>'Bina bakımı: her ev için ₺25, otel için ₺100 öde.','action'=>'repairs','house'=>25,'hotel'=>100],['text'=>'Hız cezası: ₺15 öde.','action'=>'money','value'=>-15]];
    private const CHEST_CARDS=[['text'=>'Başlangıca ilerle ve ₺200 al.','action'=>'move','value'=>0],['text'=>'Banka hatası: ₺200 al.','action'=>'money','value'=>200],['text'=>'Doktor ücreti: ₺50 öde.','action'=>'money','value'=>-50],['text'=>'Vergi iadesi: ₺20 al.','action'=>'money','value'=>20],['text'=>'Hapse git.','action'=>'jail'],['text'=>'Doğum günü: Her oyuncudan ₺10 al.','action'=>'collect_each','value'=>10]];
    ...
    public function create(array $players,array $settings):array
    {
        ...
        $chance=array_keys(self::CHANCE_CARDS);$chest=array_keys(self::CHEST_CARDS);shuffle($chance);shuffle($chest);
        return ['phase'=>'playing','turn'=>0,'players'=>$p,'properties'=>[],'houses'=>[],'mortgaged'=>[],'rolled'=>false,'dice'=>[0,0],'doubles'=>0,'pending'=>null,'auction'=>null,'trade'=>null,'freeParking'=>0,'freeParkingRule'=>!empty($settings['freeParkingPool']),'chanceDeck'=>$chance,'chestDeck'=>$chest,'lastCard'=>null,'winnerSeat'=>null,'log'=>['Monopoly başladı.']];
    }
    ...
    public static function spaces():array{return array_map(function($i,$x){$base=$x[3]??null;$cost=isset($x[2])?max(50,(int)(ceil($x[2]/100)*50)):null;return ['index'=>$i,'name'=>$x[0],'type'=>$x[1],'price'=>$x[2]??null,'group'=>$x[4]??null,'baseRent'=>$base,'buildCost'=>$x[1]==='property'?$cost:null,'rents'=>$x[1]==='property'?[$base,$base*3,$base*5,$base*7,$base*9,$base*12]:null];},array_keys(self::SPACES),self::SPACES);}
    ...
    {$pos=$s['players'][$seat]['position'];$space=self::SPACES[$pos];$type=$space[1];if(in_array($type,['property','railroad','utility'],true)){if(!isset($s['properties'][$pos])){$s['pending']=$pos;return;}$owner=$s['properties'][$pos];if($owner!==$seat&&!in_array($pos,$s['mortgaged'],true)){$rent=$this->rent($s,$pos,$dice);$s['players'][$seat]['money']-=$rent;$s['players'][$owner]['money']+=$rent;}}elseif($type==='tax'){$s['players'][$seat]['money']-=$space[2];if($s['freeParkingRule'])$s['freeParking']+=$space[2];}elseif($type==='parking'&&$s['freeParkingRule']){$s['players'][$seat]['money']+=$s['freeParking'];$s['freeParking']=0;}elseif($type==='gotojail'){$s['players'][$seat]['position']=10;$s['players'][$seat]['jail']=1;}elseif(in_array($type,['chance','chest'],true)){$this->drawCard($s,$seat,$type);}if($s['players'][$seat]['money']<0)$s['log'][]=$s['players'][$seat]['name'].' borcunu kapatmalı veya iflas etmeli.';}
    private function drawCard(array &$s,int $seat,string $type):void{$key=$type==='chance'?'chanceDeck':'chestDeck';$cards=$type==='chance'?self::CHANCE_CARDS:self::CHEST_CARDS;$s[$key]??=array_keys($cards);$index=array_shift($s[$key]);$s[$key][]=$index;$card=$cards[$index];$s['lastCard']=['seat'=>$seat,'type'=>$type,'text'=>$card['text']];if($card['action']==='money')$s['players'][$seat]['money']+=(int)$card['value'];elseif($card['action']==='move'){$old=$s['players'][$seat]['position'];$s['players'][$seat]['position']=(int)$card['value'];if($old>0)$s['players'][$seat]['money']+=200;}elseif($card['action']==='back')$s['players'][$seat]['position']=($s['players'][$seat]['position']-(int)$card['value']+40)%40;elseif($card['action']==='jail'){$s['players'][$seat]['position']=10;$s['players'][$seat]['jail']=1;}elseif($card['action']==='repairs'){$houses=0;$hotels=0;foreach($s['players'][$seat]['properties'] as $property){$level=(int)($s['houses'][$property]??0);$level>=5?$hotels++:$houses+=$level;}$s['players'][$seat]['money']-=$houses*(int)$card['house']+$hotels*(int)$card['hotel'];}elseif($card['action']==='collect_each')foreach($this->active($s) as $other)if($other!==$seat){$amount=min((int)$card['value'],$s['players'][$other]['money']);$s['players'][$other]['money']-=$amount;$s['players'][$seat]['money']+=$amount;}$s['log'][]=$s['players'][$seat]['name'].': '.$card['text'];}
