...
final class MonopolyEngine
{
    ...
    private function auctionBid(array &$s,int $seat,int $amount):void{if(!$s['auction']||$s['auction']['next']!==$seat)throw new RuntimeException('Açık artırmada sıra sizde değil.');$a=&$s['auction'];if($amount>$a['bid']&&$amount<=$s['players'][$seat]['money']){$a['bid']=$amount;$a['leader']=$seat;}else{$a['active']=array_values(array_diff($a['active'],[$seat]));}if(count($a['active'])<=1){$winner=$a['leader'];if($winner!==null){$s['players'][$winner]['money']-=$a['bid'];$s['players'][$winner]['properties'][]=$a['space'];$s['properties'][$a['space']]=$winner;}$s['auction']=null;return;}$i=array_search($seat,$a['active'],true);$a['next']=$a['active'][(($i===false?-1:$i)+1)%count($a['active'])];}
