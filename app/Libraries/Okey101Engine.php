...
final class Okey101Engine
{
    ...
    {
        if(count($tiles)<3)return -1;$wild=array_values(array_filter($tiles,fn($t)=>$this->wild($t,$joker)));$normal=array_map(fn($t)=>$this->representedTile($t,$joker),array_values(array_filter($tiles,fn($t)=>!$this->wild($t,$joker))));if(!$normal)return -1;
        ...
        if(count(array_unique($colors))===1){sort($numbers);if(count(array_unique($numbers))!==count($numbers))return -1;$missing=0;for($i=1;$i<count($numbers);$i++)$missing+=max(0,$numbers[$i]-$numbers[$i-1]-1);$wildCount=count($wild);if($missing<=$wildCount&&($numbers[count($numbers)-1]-$numbers[0]+1+$wildCount)<=13){$sum=array_sum($numbers);for($i=1;$i<count($numbers);$i++)for($n=$numbers[$i-1]+1;$n<$numbers[$i];$n++)$sum+=$n;$remaining=$wildCount-$missing;$above=min($remaining,13-$numbers[count($numbers)-1]);for($n=1;$n<=$above;$n++)$sum+=$numbers[count($numbers)-1]+$n;$remaining-=$above;for($n=1;$n<=$remaining;$n++)$sum+=$numbers[0]-$n;return $sum;}}
        return -1;
    }

    private function areFivePairs(array $tiles,array $joker):bool{$counts=[];$wild=0;foreach($tiles as $t){if($this->wild($t,$joker)){$wild++;continue;}$represented=$this->representedTile($t,$joker);$key=$represented['color'].':'.$represented['number'];$counts[$key]=($counts[$key]??0)+1;}$pairs=0;$singles=0;foreach($counts as $count){$pairs+=intdiv($count,2);$singles+=$count%2;}return $pairs+min($wild,$singles)+intdiv(max(0,$wild-$singles),2)>=5;}
    private function finishRound(array &$s,?int $winner,string $type):void{$s['phase']='completed';$s['winnerSeat']=$winner;$s['finishType']=$type;foreach($s['hands'] as $seat=>$hand){if($winner===$seat)continue;$penalty=0;foreach($hand as $t){$represented=$this->representedTile($t,$s['joker']);$penalty+=$this->wild($t,$s['joker'])?101:(int)$represented['number'];}if(!$s['opened'][$seat]&&!$s['pairOpened'][$seat])$penalty*=2;$s['scores'][$seat]+=$penalty;}if($winner!==null&&$type==='hand')foreach($s['scores'] as $seat=>&$score)if($seat!==$winner)$score*=2;}
    ...
    private function representedTile(array $tile,array $joker):array{if(empty($tile['fake']))return $tile;return ['id'=>$tile['id'],'color'=>$joker['color'],'number'=>(int)$joker['number']===1?13:(int)$joker['number']-1,'fake'=>true];}
