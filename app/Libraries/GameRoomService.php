...
class GameRoomService
{
    ...
    public function create(int $userId, string $game, string $difficulty): array
    {
        ...
        if (
            ! in_array($game, ['sudoku', 'minesweeper'], true)
            || ! SudokuPuzzles::has($difficulty)
        ) {
        ...
        $state = match ($game) {
            ...
            default => $this->newMines($difficulty),
        };
    ...
    public function versionForPlayer(string $code, int $userId): array
    {
        ...
        $room = (new GameRoomModel())
            ->select(
                'id, code, host_user_id, guest_user_id, status, version'
            )
        ...
        try {
            ...
            if ($room['status'] !== 'playing') {
                throw new RuntimeException(
                    'Oyun henüz başlamadı veya tamamlandı.'
                );
            }
            ...
            if ($room['game'] === 'sudoku') {
                ...
                throw new RuntimeException(
                    'Desteklenmeyen multiplayer oyun.'
                );
            }

            $status = ! empty($state['completed'])
                ? 'completed'
                : 'playing';
            ...
            $fresh = (new GameRoomModel())
                ->withPlayers(strtoupper($code));
    ...
    public function cleanupInactiveRooms(bool $force = false): void
    {
        ...
        if (
            ! $force
            && $cache->get('game_rooms_presence_cleanup')
        ) {
            return;
        }
        ...
        $cutoff = date(
            'Y-m-d H:i:s',
            strtotime('-25 seconds')
        );

        $roomQuery = (new GameRoomModel())
            ->select(
                'id, status, guest_user_id, host_room_seen_at, guest_room_seen_at, updated_at'
            )
            ->whereIn(
                'status',
                [
        ...
        foreach ($rooms as $room) {
            $hostGone =
                empty($room['host_room_seen_at'])
                || $room['host_room_seen_at'] < $cutoff;

            $guestGone =
                empty($room['guest_user_id'])
                || empty($room['guest_room_seen_at'])
                || $room['guest_room_seen_at'] < $cutoff;

            $completedCanBeDeleted =
                $room['status'] === 'completed'
                && (
                    ($hostGone && $guestGone)
                    || (
                        ! empty($room['updated_at'])
    ...
    private function newSudoku(string $difficulty): array
    {
        ...
        return [
            ...
            'values' => str_split($data['puzzle']),
            'owners' => array_fill(0, 81, null),
            'mistakes' => 0,
            'failed' => false,
            'startedAt' => null,
            'completed' => false,
    ...
    private function revealMineCells(
        array &$state,
        int $userId,
        int|array $indexes
    ): void {
        $queue = is_array($indexes)
        ...
        while (isset($queue[$cursor])) {
            ...
            if (
                isset($revealedLookup[$current])
                || isset($flagLookup[$current])
                || isset($mineLookup[$current])
            ) {
                continue;
            }
    ...
    private function adjacentMineCount(
        array $state,
        int $index
    ): int {
        return count(
    ...
    private function publicRoom(
        ...
        if ($room['game'] === 'minesweeper') {
            ...
            unset($state['mineIndexes']);

            $state['numbers'] = [];

            foreach (
                $state['revealed'] as $index
            ) {
                $state['numbers'][
                    (string) $index
                ] = count(
    ...
    private function touchRoomPresence(
        int $roomId,
        int $hostUserId,
        int $userId
    ): void {
