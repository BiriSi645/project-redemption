...
<section class="time-shell" aria-labelledby="time-title">
    ...
    <div class="time-tabs" role="tablist" aria-label="Zaman aracı seçimi">
        <button id="stopwatch-tab" class="time-tab active" type="button" role="tab"
            aria-selected="true" aria-controls="stopwatch-panel">Kronometre</button>
        <button id="countdown-tab" class="time-tab" type="button" role="tab" aria-selected="false"
            aria-controls="countdown-panel">Zamanlayıcı</button>
    </div>
    ...
    <div id="stopwatch-panel" class="time-panel" role="tabpanel" aria-labelledby="stopwatch-tab">
        ...
        <div class="time-actions">
            ...
            <button id="stopwatch-pause" class="time-action pause" type="button"
                disabled>Duraklat</button>
            <button id="stopwatch-lap" class="time-action lap" type="button" disabled>Tur
                Kaydet</button>
            <button id="stopwatch-reset" class="time-action reset" type="button"
                disabled>Sıfırla</button>
        </div>
        <div class="laps">
            <div class="laps-header">
                <h2>Turlar</h2>
                <button id="clear-laps" class="clear-laps" type="button" hidden>Turları
                    Temizle</button>
            </div>
    ...
    <div id="countdown-panel" class="time-panel" role="tabpanel" aria-labelledby="countdown-tab"
        hidden>
        <p class="time-description">Süreyi belirleyin; zaman dolduğunda sesli ve görsel bildirim
            alın.</p>
        <div class="countdown-inputs">
            <label>Saat<input id="countdown-hours" type="number" min="0" max="99" value="0"
                    inputmode="numeric"></label>
            <label>Dakika<input id="countdown-minutes" type="number" min="0" max="59" value="5"
                    inputmode="numeric"></label>
            <label>Saniye<input id="countdown-seconds" type="number" min="0" max="59" value="0"
                    inputmode="numeric"></label>
        </div>
        <div id="countdown-display" class="time-display" role="timer" aria-live="polite">00:05:00
        </div>
        <div class="time-actions">
            <button id="countdown-start" class="time-action start" type="button">Başlat</button>
            <button id="countdown-pause" class="time-action pause" type="button"
                disabled>Duraklat</button>
...
<script>
    (() => {
        ...
        function loadStopwatch() {
            ...
            try {
                ...
                return {
                    ...
                    running: saved.running === true && Number.isFinite(saved.startedAt),
        ...
        const stopwatchNow = () => swState.elapsed + (swState.running && swState.startedAt ?
            Date.now() - swState.startedAt : 0);
        ...
        function renderStopwatchClock() {
            ...
            if (swState.running && !document.hidden) swFrame = requestAnimationFrame(
        ...
        swPause.addEventListener('click', () => {
            swState.elapsed = stopwatchNow();
            swState.startedAt = null;
            swState.running = false;
            saveStopwatch();
            renderStopwatch();
        });
        swLap.addEventListener('click', () => {
            swState.laps.push(stopwatchNow());
            saveStopwatch();
            renderStopwatch();
        });
        swReset.addEventListener('click', () => {
            if (confirm('Kronometre ve turlar sıfırlansın mı?')) {
                swState = {
                    elapsed: 0,
                    startedAt: null,
                    running: false,
        ...
        function loadCountdown() {
            ...
            try {
                ...
                if (!saved || typeof saved.duration !== 'number' || typeof saved.remaining !==
                    'number') return fallback;
                return {
                    duration: Math.max(0, saved.duration),
                    remaining: Math.max(0, saved.remaining),
                    endAt: Number.isFinite(saved.endAt) ? saved.endAt : null,
                    running: saved.running === true && Number.isFinite(saved.endAt),
        ...
        const countdownNow = () => cdState.running && cdState.endAt ? Math.max(0, cdState
            .endAt - Date.now()) : cdState.remaining;
        ...
        function playAlarm() {
            try {
                ...
                /* Ses desteği yoksa görsel bildirim devam eder. */ }
        }
        ...
        function renderCountdown() {
            ...
            cdStart.textContent = remaining > 0 && remaining < cdState.duration && !cdState
                .running ? 'Devam Et' : 'Başlat';
            cdStart.disabled = cdState.running;
            cdPause.disabled = !cdState.running;
            [cdHours, cdMinutes, cdSeconds].forEach(input => {
                input.disabled = cdState.running || (remaining < cdState.duration &&
                    remaining > 0);
            });
            if (cdState.running && !document.hidden) cdInterval = setInterval(tickCountdown,
                1000);
        }
        ...
        cdStart.addEventListener('click', () => {
            if (cdState.finished || cdState.remaining <= 0 || cdState.remaining ===
                cdState.duration) {
                cdState.duration = durationFromInputs();
                cdState.remaining = cdState.duration;
            }
            if (cdState.remaining <= 0) {
                cdMessage.textContent = 'Lütfen sıfırdan büyük bir süre girin.';
                return;
            }
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                ...
                renderStopwatch();
                if (cdState.running && countdownNow() <= 0) finishCountdown();
                else renderCountdown();
            }
        });

        updateInputs(cdState.duration);
        selectTab(localStorage.getItem(activeTabKey) === 'countdown' ? 'countdown' :
            'stopwatch');
        renderStopwatch();
        if (cdState.running && countdownNow() <= 0) finishCountdown();
        else renderCountdown();
