...
<style>
    ...
    html[data-theme="dark"] .visibility-badge.private {
        background: #0f172a;
        border-color: #334155
    }
    ...
    html[data-theme="dark"] .feed-tab:hover,
    html[data-theme="dark"] .feed-tab.active {
        background: #334155;
        color: #93c5fd
    }
    ...
    html[data-theme="dark"] .note-action {
        color: #cbd5e1
    }

    html[data-theme="dark"] .note-action:hover {
        background: #334155;
        color: #fff
    }

    html[data-theme="dark"] .visibility-badge.public {
        background: #14532d;
        color: #bbf7d0
    }
...
$scopeUrl = static fn(string $scope): string => site_url("notes") .
    "?" .
...
<div class="notes-page">
    <header class="notes-hero">
        <div>
            ...
            <div class="notes-intro"><?= $isAdmin
                ? "Topluluktaki paylaşımları ve özel notları yönetin."
                : "Fikirlerinizi paylaşın, diğer kullanıcıların notlarını keşfedin." ?></div>
        </div>
    ...
    <nav class="feed-tabs" aria-label="Not görünümü">
        ...
        <a class="feed-tab <?= $activeScope === $scope ? "active" : "" ?>" href="<?= esc(
    $scopeUrl($scope),
    "attr",
) ?>" <?= $activeScope === $scope ? 'aria-current="page"' : "" ?>><?= esc($label) ?></a>
        <?php endforeach; ?>
    </nav>
    ...
    <form class="content-filter notes-filter" method="get" action="<?= site_url("notes") ?>">
        ...
        <input type="search" name="q" value="<?= esc(
            $search,
        ) ?>" placeholder="Akışta ara…" aria-label="Notlarda ara">
        <select name="category" aria-label="Kategori">
            <option value="">Tüm kategoriler</option>
            <?php foreach ($categories as $category): ?>
            <option value="<?= esc($category, "attr") ?>" <?= $activeCategory === $category
    ? "selected"
    : "" ?>><?= esc($category) ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="notes-feed">
        ...
        <article class="note-card">
            <header class="note-card-head">
                <a class="note-author" href="<?= site_url(
                            "users/" . $note["user_id"],
                        ) ?>" style="text-decoration:none">
                    <span class="author-avatar" aria-hidden="true"><?= esc(
                                $initial,
                            ) ?></span>
                    <div class="author-details">
                        <strong class="author-name"><?=
                                ...
                                ($isOwner ? " · Siz" : "")
                                ?></strong>
                        <div class="note-meta"><?= $createdAt
                                    ? esc(date("d.m.Y · H:i", $createdAt))
                                    : "Tarih bilinmiyor" ?></div>
                    </div>
                </a>
                <span class="visibility-badge <?= (int) $note["is_public"] === 1
                            ? "public"
                            : "private" ?>" title="<?= (int) $note["is_public"] === 1
    ? "Tüm kullanıcılar görebilir"
    : "Yalnızca siz ve admin görebilir" ?>">
                    <span aria-hidden="true"><?= (int) $note["is_public"] === 1
                                ? "◉"
                                : "●" ?></span>
                    <span class="visibility-label"><?= (int) $note["is_public"] === 1
                                ? "Herkese açık"
                                : "Özel" ?></span>
                </span>
            </header>
            ...
            <a class="note-card-body" href="<?= site_url("notes/" . $note["id"]) ?>">
                ...
                <p class="note-preview"><?= esc(
                            mb_strimwidth($note["content"], 0, 360, "…"),
                        ) ?></p>
                <span class="note-category">#<?= esc(
                            str_replace(" ", "", $note["category"] ?? "Genel"),
                        ) ?></span>
            </a>

            <footer class="note-card-footer">
                <div class="note-social-actions">
                    <a class="note-action" href="<?= site_url(
                                "notes/" . $note["id"],
                            ) ?>#comments" aria-label="Yorumları görüntüle">💬 <?= (int) ($note[
    "comment_count"
] ?? 0) ?> yorum</a>
                    <a class="note-action" href="<?= site_url(
                                "notes/" . $note["id"],
                ...
                <div class="note-owner-actions">
                    <?php if (
                                    $isOwner
                                ): ?><a class="note-action edit" href="<?= site_url(
    "notes/" . $note["id"] . "/edit",
) ?>">Düzenle</a><?php endif; ?>
                    <form method="post" action="<?= site_url(
                                    "notes/" . $note["id"] . "/delete",
                                ) ?>" onsubmit="return confirm('Bu not silinsin mi?')">
                        <?= csrf_field() ?><button class="note-action delete"
