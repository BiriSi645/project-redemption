...
(function(){
    ...
    function replaceWithClone(input, newValue, caretPos){
        try{
            ...
            console.log('autofillFix(reset): replacedWithClone', {newValue: newValue});
        }catch(err){ console.error('autofillFix(reset): replace error', err); }
    }
        ...
        try{
            ...
            if (inputType && (inputType.startsWith('insert') || inputType === 'deleteContentBackward' || inputType === 'deleteContentForward')) {
                ...
                if (inputType.startsWith('insert')) {
                    ...
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(reset): beforeinput applied manually', { expected });
                    ...
                    try { p.setSelectionRange(newPos, newPos); } catch (err) {}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(reset): delete backward applied manually', { expected });
                    ...
                    try { p.setSelectionRange(newPos, newPos); } catch (err) {}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(reset): delete forward applied manually', { expected });
                }
                pending = null;
            }
    ...
    function onInput(e){
        ...
        try{
            ...
            if (p.value === oldVal) {
                ...
                p.dispatchEvent(new Event('input', { bubbles: true }));
                console.log('autofillFix(reset): repaired by setting value', {expected});
                setTimeout(() => { if (p && p.value === oldVal) { replaceWithClone(p, expected, newPos); } }, 30);
            } else if (p.value !== expected) {
                console.log('autofillFix(reset): input mismatch', {value: p.value, expected});
            }
        }catch(err){ console.error('autofillFix(reset): input handler error', err); }
        pending = null;
    }
    ...
    function onPaste(e){
        ...
        try{
            ...
            setTimeout(() => {
                ...
                p.dispatchEvent(new Event('input', { bubbles: true }));
                console.log('autofillFix(reset): paste applied', {expected});
            }, 25);
        }catch(err){ console.error('autofillFix(reset): paste error', err); }
    }
...
<script>
    (function(){
        ...
        console.log('debugAutofill(reset): started', {passwordFound: !!p, location: location.href});
        if (!p) return;
        console.log('debugAutofill(reset): initial', {value: p.value, readOnly: p.readOnly, disabled: p.disabled});
        ['keydown','keypress','keyup','input','focus','blur'].forEach(evt => p.addEventListener(evt, e => {
            console.log('debugAutofill(reset) event', evt, {key: e.key, value: p.value});
        }));
        (function inspectAncestors(el){
            let cur = el;
            while(cur){
                try{ const s = getComputedStyle(cur); console.log('debugAutofill(reset) ancestor', cur.tagName, {pointerEvents: s.pointerEvents, opacity: s.opacity}); }catch(e){}
