...
<script>
    (function(){
        ...
        console.log('debugAutofill(register): started', {passwordFound: !!p, location: location.href});
        if (!p) return;
        console.log('debugAutofill(register): initial', {value: p.value, readOnly: p.readOnly, disabled: p.disabled});
        ['keydown','keypress','keyup','input','focus','blur'].forEach(evt => p.addEventListener(evt, e => {
            console.log('debugAutofill(register) event', evt, {key: e.key, value: p.value});
        }));
        (function inspectAncestors(el){
            let cur = el;
            while(cur){
                try{ const s = getComputedStyle(cur); console.log('debugAutofill(register) ancestor', cur.tagName, {pointerEvents: s.pointerEvents, opacity: s.opacity}); }catch(e){}
...
(function(){
    let p = document.getElementById('password') || document.querySelector('input[type=password]');
    if (!p) return;
    const shouldEnable = new URLSearchParams(location.search).has('autofillFix') || !!p.value;
    ...
    function onKeydown(e){ lastVal = p.value; lastKey = e.key; }
    ...
    function replaceWithClone(input, newValue, caretPos){
        try{
            ...
            attachHandlers(p);
            try { p.focus(); } catch(e){}
            try { p.setSelectionRange(caretPos, caretPos); } catch(e){}
            p.dispatchEvent(new Event('input', { bubbles: true }));
            console.log('autofillFix(register): replacedWithClone', {newValue: newValue});
        }catch(err){ console.error('autofillFix(register): replace error', err); }
    }
    ...
    function onBeforeInput(e){
        ...
        try{
            ...
            pending = { oldValue: old, data: data, inputType: inputType, start: start, end: end };
            console.log('autofillFix(register): beforeinput', pending);

            if (inputType && (inputType.startsWith('insert') || inputType === 'deleteContentBackward' || inputType === 'deleteContentForward')) {
                try { e.preventDefault(); } catch (err) {}
                if (inputType.startsWith('insert')) {
                    const expected = old.slice(0, start) + (data || '') + old.slice(end);
                    p.value = expected;
                    const newPos = start + (data ? data.length : 0);
                    try { p.setSelectionRange(newPos, newPos); } catch (err) {}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(register): beforeinput applied manually', { expected });
                    ...
                    try { p.setSelectionRange(newPos, newPos); } catch (err) {}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(register): delete backward applied manually', { expected });
                } else if (inputType === 'deleteContentForward') {
                    const expected = old.slice(0, start) + old.slice(Math.min(end + 1, old.length));
                    const newPos = start;
                    p.value = expected;
                    try { p.setSelectionRange(newPos, newPos); } catch (err) {}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(register): delete forward applied manually', { expected });
                }
                pending = null;
            }
    ...
    function onInput(e){
        if (composing) { pending = null; return; }
        if (!pending) return;
        try{
            ...
            if (p.value === oldVal) {
                ...
                try { p.setSelectionRange(newPos, newPos); } catch (e){}
                p.dispatchEvent(new Event('input', { bubbles: true }));
                console.log('autofillFix(register): repaired by setting value', {expected});
                setTimeout(() => { if (p && p.value === oldVal) { replaceWithClone(p, expected, newPos); } }, 30);
            } else if (p.value !== expected) {
                // mismatch: log for debugging
                console.log('autofillFix(register): input mismatch', {value: p.value, expected});
            }
        }catch(err){ console.error('autofillFix(register): input handler error', err); }
        pending = null;
    }
    ...
    function onPaste(e){
        if (composing) return;
        try{
            const paste = (e.clipboardData && e.clipboardData.getData) ? e.clipboardData.getData('text') : null;
            if (!paste) return;
            ...
            setTimeout(() => {
                if (p.value === p.value) {
                    p.value = expected;
                    try { p.setSelectionRange(start + paste.length, start + paste.length); } catch (e){}
                    p.dispatchEvent(new Event('input', { bubbles: true }));
                    console.log('autofillFix(register): paste applied', {expected});
                }
            }, 25);
        }catch(err){ console.error('autofillFix(register): paste error', err); }
    }
...
<script>
    ...
    (function () {
        function ensureEditable(input) {
            ...
            try { input.setSelectionRange(input.value.length, input.value.length); input.dispatchEvent(new Event('input', { bubbles: true })); return; } catch (e) {}
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('input[type="password"]').forEach(i => initial.set(i, i.value));
            let tries = 0;
