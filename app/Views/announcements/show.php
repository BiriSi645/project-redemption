...
<style>
    ...
    html[data-theme="dark"] .announcement-badge.update {
        background: #064e3b;
        color: #d1fae5
    }
</style>
<div class="announcement-wrap">
    <article class="announcement-card">
        <span class="announcement-badge <?= $announcement["type"] === "update"
        ? "update"
        : "" ?>"><?= $announcement["type"] === "update"
    ? "Güncelleme notu"
    : "Genel duyuru" ?></span>
        <h1><?= esc($announcement["title"]) ?></h1>
        <div class="announcement-meta"><?= esc(
        $announcement["author_username"] ?? "Yönetim",
    ) ?> · <?= date("d.m.Y · H:i", strtotime($announcement["created_at"])) ?></div>
        <div class="announcement-content"><?= esc($announcement["content"]) ?></div>
        <div class="announcement-actions">
            <?php if (
            !empty($announcement["target_path"]) &&
            preg_match('#^[a-zA-Z0-9/_-]+$#', $announcement["target_path"])
        ): ?><a class="button" href="<?= site_url(
    $announcement["target_path"],
) ?>">İlgili sayfayı aç</a><?php endif; ?>
