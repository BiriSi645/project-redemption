...
<style>
    ...
    html[data-theme="dark"] .multi-card {
        background: #1e293b;
        border-color: #334155
    }

    html[data-theme="dark"] .multi-card select,
    html[data-theme="dark"] .multi-card input {
        background: #0f172a;
        border-color: #475569;
        color: #e2e8f0
    }
...
<div class="multi-page">
    ...
    ) ?>"><?= csrf_field() ?><h2>Yeni oda</h2>
            <p>Oyunu ve zorluğu seçip özel bir oda kodu alın.</p><label
    ...
    ) ?>"><?= csrf_field() ?><h2>Odaya katıl</h2>
            <p>Arkadaşınızın gönderdiği altı karakterli oda kodunu yazın.</p><label
                for="room-code">Oda kodu</label><input class="room-code-input" id="room-code"
                name="code" maxlength="6" minlength="6" autocomplete="off" required value="<?= esc(
    old("code") ?? "",
) ?>"><button class="button" type="submit">Odaya katıl</button>
        </form>
    </div>
</div>
<?= $this->endSection() ?>