...
class DirectConversationModel extends Model
{
    ...
    public function forUser(int $userId, int $perPage = 20): array
    {
        ...
        return $this->select('direct_conversations.*')
            ...
            ->select("CASE WHEN user_one_id = {$escaped} THEN user_two.username ELSE user_one.username END AS other_username", false)
            ->select("(SELECT body FROM direct_messages dm WHERE dm.conversation_id = direct_conversations.id AND ((dm.sender_id = {$escaped} AND dm.deleted_by_sender = 0) OR (dm.sender_id != {$escaped} AND dm.deleted_by_recipient = 0)) ORDER BY dm.id DESC LIMIT 1) AS last_body", false)
            ->select("(SELECT COUNT(*) FROM direct_messages dm WHERE dm.conversation_id = direct_conversations.id AND dm.sender_id != {$escaped} AND dm.read_at IS NULL AND dm.deleted_by_recipient = 0) AS unread_count", false)
            ->join('users user_one', 'user_one.id = direct_conversations.user_one_id')
            ->join('users user_two', 'user_two.id = direct_conversations.user_two_id')
